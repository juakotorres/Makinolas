...
public class WaterPlatform extends GameActor {
	...
	public WaterPlatform(World myWorld, int x, int y, int widthTiles, int heightTiles) {
		...
		/*
		 * k es el indice de que tan rapido se mueve el agua, entre mas alto mas
		 * lento, 0 es lo mas rapido
		 */
		k = 3;

		myBodyDefinition = new BodyDef();
		myBodyDefinition.position.set(new Vector2(x * TILE_FACTOR + widthTiles * TILE_FACTOR / 2,
				y * TILE_FACTOR + heightTiles * TILE_FACTOR / 2));
		...
		/*
		 * Fixture atravesable de la plataforma para detectar cuando entra y
		 * sale del agua
		 */
	...
	@Override
	  public void interactWithHero(IHero hero, WorldManifold worldManifold) {
		...
		hero.getBody().setGravityScale(0.5f);

		/*
		 * damping para frenar al hero cuando entra al agua y cuando se mueve en
		 * general
		 */
	...
	public void endHeroInteraction(Hero hero, WorldManifold worldManifold) {
		...
		/*
		 * setea que tan rapido cae luego de salir del agua antes de saltar,
		 * cuando esta en la superficie
		 */
